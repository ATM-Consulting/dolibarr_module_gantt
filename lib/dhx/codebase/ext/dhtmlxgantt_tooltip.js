...
$( document ).on( "mousemove", function( event ) {
	ganttMouseX = event.pageX;
	ganttMouseY = event.pageY;
});
...
/*	pos.x += (gantt.config.tooltip_offset_x*1 || 0);
	...
	if($(document).scrollTop()>0) pos.y+=$(document).scrollTop();
...
*/
	tip.style.left = mouse_pos.x + "px";
	tip.style.top  = mouse_pos.y + "px";
